...
class RemindWriteContentActivity : AppCompatActivity(), RemindDate {
    ...
    private fun loadData() {
        ...
        repository
            .getRemindDetail(remindId,
                {
                    etRemindWriteContent.apply {
                        if (!it.contents.isBlank()) {
                            setText(it.contents)
    ...
    private fun saveContents(id: Int) {
        repository
            .saveRemind(etRemindWriteContent.text.toString(), id, null,
                {},
                { Toast.makeText(this, it, Toast.LENGTH_SHORT).show() })
    }
    ...
    private fun toolbarClick() {
        ...
        tvCompleteToolbarRemind.setOnClickListener {
            MaterialDialog(this).show {
                message(
                    text = getString(
                        R.string.toolbar_remind_dialog_msg,
                        getMonth(startDate),
                        convertKorean(getWeekOfMonth(startDate))
                    )
                )
                positiveButton(text = "확인") {
                    saveOrUpdateContents(remindId)
                    val intent = Intent(it.windowContext, RemindListActivity::class.java).apply {

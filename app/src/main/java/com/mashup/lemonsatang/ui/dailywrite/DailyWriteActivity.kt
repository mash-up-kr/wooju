...
class DailyWriteActivity : AppCompatActivity() {
    ...
    private fun initializeData() {
        ...
        if(intent.getIntExtra("dailylogId",-1) != -1){// DailyViewActivity 에서 edit 버튼을 눌러 들어온 경우
            ...
            rv_daily_write.layoutManager!!.scrollToPosition(
                intent.getIntExtra(
                    "emotionId",
                    0
                )
    ...
    private fun setToolbarAndDailyDate() {
        ...
        //daily date 설정
        tv_daily_date.text = "${writeMonth}월 ${writeDay}일 ${convertToDayOfWeek(
            writeYear,
            writeMonth - 1,
            writeDay
        )}요일"
    ...
    private fun clickTvDailyDate() {
        ...
        tv_daily_date.setOnClickListener {
            DatePickerDialog(
                ...
                intent.getIntExtra(CURR_YEAR_KEY, -1),
                intent.getIntExtra(CURR_MONTH_KEY, -1) - 1,
                intent.getIntExtra(CURR_DAY_KEY, -1)
            ).show()
        }
    }
    ...
    private fun postOrPutData() {
        var dailyWriteContent = et_daily_content.text.toString() //내용 저장
        var dailyWriteEmotion = rv_daily_write.currentItem //감정 저장
        ...
        if (!isEdit) {
            ...
            //수정 view 인 경우 put
            val articleDto = Article(
                dailyWriteContent,
                intent.getIntExtra("dailylogId", -1),
                dailyWriteEmotion,
                timeFormatter()
            )
    ...
    private fun timeFormatter(): String {
        val writeDate =
            SimpleDateFormat("yyyy-MM-dd").parse("${writeYear}-${writeMonth}-${writeDay}")
        val currentTime =
            SimpleDateFormat("HH:mm:ss.SSS").format(Date()) // 지난 날짜의 기록을 남길 경우 현재 시간으로 설정했음
